...
\begin{document}
\tutorialtitle{Note 2 Code Running Guide}{PINN/PIDL Cyber-Control Examples}{Installation, smoke tests, training diagnostics, outputs, and troubleshooting}{Companion to Note 2: Physics-Informed Cyber Control}
\tableofcontents
\newpage
\lhead{\small\sffamily Note 2 Code Running Guide}
...
\subsection{Heterogeneous node-SIPS inverse PINN}
\begin{lstlisting}[style=pythonstyle]
python src/node_sips_inverse_pinn.py --smoke --device cpu
\end{lstlisting}
...
\item diagnostic figures under \texttt{artifacts/figures/}, including the
training-diagnostics and baseline-comparison panels.
\end{itemize}
...
\section{Output Map}
Start with \texttt{artifacts/experiments/OUTPUT\_PREVIEW.md}. It groups the generated results by experiment, diagnostic panel, baseline-comparison panel, and file. Then inspect the generated figures in \texttt{artifacts/figures/} and the CSV file for the experiment you are studying.
...
\texttt{baseline\_comparison}\newline\texttt{.png} & Learned methods compared with sparse interpolation, wrong-parameter SIR, known-SIR-only dynamics, and no/fixed-control baselines. \\
\bottomrule
\end{longtable}
...
Move gradually: first degree-level compartments, then node-level vectors or graph-neural PINNs. For stiff, impulsive, or continuous-impulsive dynamics, use piecewise networks and add collocation points near jump times.
